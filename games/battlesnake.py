...
class Game:
    ...
    def __init__(self, height=11, width=11, snake_cnt=4, health_dec=1, food_spawn_chance=0.15, show=False):
        self.height = height
        self.width = width
        ...
        self.show = show
        if show:
            self.replay_file = "replay_battlesnake.rep"
    ...
    """
    def get_states(self):
        return [self.get_state(snake, self.last_moves[snake.id]) for snake in self.snakes]

    """
    Get game and snake ids
    Return:
        a list of snake's id
    """
    def get_ids(self):
        return [snake.id for snake in self.snakes]

    """
    Get legal actions for each snake
    Return:
        lists of actions for each snake
    """
    def get_legal_actions(self):
        return [(0, 1, 2) for snake in self.snakes]
    ...
    def tic(self, moves):
        ...
        for i in range(len(snakes)):
            snake = snakes[i]
            # convert relative move direction to absolute direction
    ...
    """
    def get_state(self, you, last_move):
        # gotta do the math to recenter the grid
        width = self.width*2 - 1
        height = self.height*2 - 1
        health = you.health*HEALTH_m
        grid = [[[0.0, WALL, 0.0, health] for col in range(width)] for row in range(height)]
        center_y = height//2
        center_x = width//2
        # the original game board
        # it's easier to work on the original board then transfer it onto the grid
        board = [[[0.0, 0.0, 0.0, health] for col in range(self.width)] for row in range(self.height)]
    ...
    """
    def copy(self):
        game = Game(self.height, self.width, self.snake_cnt, self.health_dec, self.food_spawn_chance)
...
class Snake:
    ...
    def __init__(self, ID, health, head_and_body):
        ...
        self.head = SnakeNode(head_and_body[0])
        self.tail = self.head
        for i in range(1, len(head_and_body)):
            new_node = SnakeNode(head_and_body[i])
    ...
    def grow(self):
        self.length += 1
        new_tail = SnakeNode(self.tail.position)
    ...
    def copy(self):
        ...
        snake.head = SnakeNode(self.head.position)
        snake.tail = snake.head
        curr = self.head.next_node
        while curr:
            new_node = SnakeNode(curr.position)
